...
    int size,my_rank,nfila,ncolumna;
...
    MPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
    ...
    if(my_rank == 0){
    do{
    cout<<"Cantidad de filas (1-"<<size<<"): ";
    cin>>nfila;
    if(nfila>size)
	cout<<"Cantidad excedida a procesadores\n";
    }while(nfila>size);
    cout<<"Cantidad de columnas: ";
    cin>>ncolumna;
    MPI_Bcast(&nfila,1,MPI_INT,0,MPI_COMM_WORLD);
    MPI_Bcast(&ncolumna,1,MPI_INT,0,MPI_COMM_WORLD);
    }
    else{
    MPI_Bcast(&nfila,1,MPI_INT,0,MPI_COMM_WORLD);
    MPI_Bcast(&ncolumna,1,MPI_INT,0,MPI_COMM_WORLD);
...
    if (my_rank == 0) {
        A[0] = new long [nfila * ncolumna];
        for (unsigned int i = 1; i < nfila; i++) {
            A[i] = A[i - 1] + ncolumna;
        }
...
        for (unsigned int i = 0; i < nfila; i++) {
            for (unsigned int j = 0; j < ncolumna; j++) {
                if (j == 0) cout << "[";
                A[i][j] = rand() % 10;
                cout << A[i][j];
                if (j == ncolumna - 1){
...
            }
        }
        for (unsigned int k = nfila; k < ncolumna; k++) {
        if(k<ncolumna){
           x[k] = rand() % 10;
           cout << "\t\t  [" << x[k] << "]";
           cout<<endl;
...
            ncolumna, // Numero de columnas a compartir
...
            ncolumna, // Numero de elementos que se van a enviar y recibir
...
    long subFinal = 0;
    for (unsigned int i = 0; i < ncolumna; i++) {
